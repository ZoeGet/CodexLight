...
| 电池供电 | ETA6093S2F 负责单节锂电池开关充电和 5 V 同步升压，使用 2.2 µH 电感、22 µF 输出电容、MX1.25 电池接口和电源开关 |
...
ETA6093S2F 的 `LED` 引脚通过 R5（1 kΩ）连接 LED1、LED2，用于显示充电和升压工作状态；L1（2.2 µH）连接 `SW` 与 `VBAT`，C4（22 µF）用于 +5 V 输出滤波，C3（10 µF）用于电池侧去耦。电池必须核对接口极性并带有保护电路。更详细的电路说明和组装注意事项见 [开源平台项目说明](Docs/立创开源平台项目说明.md)。
