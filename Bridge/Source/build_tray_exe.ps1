...
$bridgeDir = Split-Path -Parent $sourceDir
...
$launcherSource = Join-Path $sourceDir "CodexLightLauncher.cs"
$trayScript = Join-Path $sourceDir "CodexLightTray.ps1"
$monitorScript = Join-Path $sourceDir "codex_light_monitor.py"
$compilerArguments = @(
  "/nologo",
  "/target:winexe",
  "/platform:x64",
